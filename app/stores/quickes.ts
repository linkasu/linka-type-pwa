  ...
  actions: {
    async fetchQuickes() {
      ...
      try {
        ...
        if (response.quickes && Array.isArray(response.quickes)) {
          this.quickes = response.quickes
        }
      } catch (err: unknown) {
        const error = err as Error
        this.error = error.message || 'Failed to fetch quickes'
        console.warn('Failed to fetch quickes, using defaults:', error.message)
        // Keep defaults on error - quickes stay as DEFAULT_QUICKES
      } finally {
